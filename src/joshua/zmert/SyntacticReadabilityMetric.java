...
public class SyntacticReadabilityMetric extends GradeLevelMetric{
    ...
    private static GradeLevelMetric gradeLevel;
    ...
    */
	...
	initialize();
	/*		try {
			loadLinesFromFile(options[0],srcSentences);
			loadLinesFromFile(options[1],srcParses);
			} catch (IOException e) {
			System.err.println("Error loading the source sentences from "+options[0]+" and "+options[1]);
			e.printStackTrace();
			System.exit(1);
			}
	*/
    ...
    private void loadLinesFromFile(String filepath, String[] a) throws IOException {
	BufferedReader br = new BufferedReader(new FileReader(filepath));
	String line;
	a = new String[numSentences];
	int i = 0;
	...
	//		stats[gl_stats.length+2] = tree.getLeaves().size();
