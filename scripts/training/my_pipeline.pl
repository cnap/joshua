...
#
# Taken directly from joshua devel 2012-2-8 and edited
...
BEGIN {
  ...
  $JOSHUA = "/Users/courtney/tools/joshua_court";
...
# get the metric name for evaluation purposes
$METRIC =~ /(\S+)\s/;
$METRIC_NAME = lc $1;
if ($METRIC_NAME ne "bleu") { $NOT_BLEU = 1; }
...
if ($METRIC_NAME =~ /syn/) {
  $MERTFILES{'decoder_command'} = "$MERTCONFDIR/decoder_command.parsing";
...
system("cp $GLUE_GRAMMAR_FILE $DATA_DIRS{test}/grammar.glue");
$GLUE_GRAMMAR_FILE = "$DATA_DIRS{test}/grammar.glue";

  if ($GLUE_GRAMMAR_FILE =~ /^\//) {
#	system("cp $GLUE_GRAMMAR_FILE $filename"); 
  } else {
#	system("cp ../../$GLUE_GRAMMAR_FILE $filename"); 
  }
}
...
for my $run (1..$OPTIMIZER_RUNS) {
  ...
  # system("cat $testrun/test.output.1best.$METRIC_NAME");
}

# Now average the runs, report metric score
my @bleus;
my $numrecs = 0;
my $dir = (defined $NAME) ? "test/$NAME" : "test";
open CMD, "grep ' ".uc $METRIC_NAME." = ' $dir/*/*$METRIC_NAME |";
while (<CMD>) {
  my @F = split;
  push(@bleus, $F[-1]);
}
...
open BLEU, ">$dir/final-$METRIC_NAME" or die "Can't write to $dir/final-$METRIC_NAME";
printf(BLEU "%s / %d = %.4f\n", join(" + ", @bleus), scalar @bleus, $final_bleu);
close(BLEU);

system("cat $dir/final-$METRIC_NAME");
exit;
...
$numrefs = get_numrefs($TEST{target});
$cachepipe->cmd("$NAME-test-$METRIC_NAME",
				"java -cp $JOSHUA/bin -Dfile.encoding=utf8 -Djava.library.path=lib -Xmx1000m -Xms1000m -Djava.util.logging.config.file=logging.properties joshua.util.JoshuaEval -cand $testrun/test.output.1best -ref $TEST{target} -rps $numrefs -m $METRIC$source_dir > $testrun/test.output.1best.$METRIC_NAME",
				"$testrun/test.output.1best", 
				"$testrun/test.output.1best.$METRIC_NAME");
